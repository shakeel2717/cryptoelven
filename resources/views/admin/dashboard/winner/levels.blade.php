...
@section('title', '1st & 2nd Level business Winner')
@section('head')
                ...
                <div class="card border-theme bg-theme bg-opacity-5 mb-3">
                    ...
                    <div class="card-body">
                        <table class="table text-nowrap w-100" id="datatableDefault">
                            ...
                            <tbody>
                                @forelse ($winners as $user)
                                    <tr>
                                        <td class="text-center">{{ $loop->iteration }}</td>
                                        <td class="text-center text-capitalize">{{ $user->name }}</td>
                                        <td class="text-center text-capitalize">{{ $user->username }}</td>
                                        <td class="text-center text-capitalize">{{ number_format(levelsSellBusiness($user->id,12000),2) }}/-</td>
                                    </tr>
                                @empty
                                    <p>No Record Found</p>
                                @endforelse
