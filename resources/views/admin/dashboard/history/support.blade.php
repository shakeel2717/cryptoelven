                            ...
                            <tbody>
                                @forelse ($statement as $transaction)
                                    <tr>
                                        ...
                                        <td class="text-center">{{ $transaction->subject }}</td>
                                        <td class="text-center">{{ $transaction->message }}</td>
                                        <td class="text-center text-uppercase"><span
                                                class="badge badge-danger">{{ $transaction->status }}</span></td>
                                        <td class="text-center">{{ $transaction->created_at }}</td>
                                        @if ($transaction->status == 'pending')
                                            <td class="text-center"><a
                                                    href="{{ route('admin.history.user.support.solved', ['id' => $transaction->id]) }}"
                                                    class="btn btn-success btn-sm">Solved</a>
                                            @else
                                            <td class="text-center"><a href="#" class="btn btn-dark btn-sm"> <i
